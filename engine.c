...
int fA(int q_sq,int s_attacker_side){char c;int t,i,d;char ap,an,ak,ar,ab,aq;if(s_attacker_side==0){ap=_WP;an=_WN;ak=_WK;ar=_WR;ab=_WB;aq=_WQ;}else{ap=_BP;an=_BN;ak=_BK;ar=_BR;ab=_BB;aq=_BQ;}if(s_attacker_side==0){if(gB[q_sq-11]==ap||gB[q_sq-9]==ap)return 1;}else{if(gB[q_sq+11]==ap||gB[q_sq+9]==ap)return 1;}for(i=0;i<8;i++){t=q_sq+dN[i];if(gB[t]==an)return 1;}for(i=0;i<8;i++){t=q_sq+dK[i];if(gB[t]==ak)return 1;}for(i=0;i<4;i++){d=dR[i];t=q_sq+d;while(gB[t]!=_O){c=gB[t];if(c!=_E){if(c==ar||c==aq)return 1;break;}t+=d;}}for(i=0;i<4;i++){d=dB[i];t=q_sq+d;while(gB[t]!=_O){c=gB[t];if(c!=_E){if(c==ab||c==aq)return 1;break;}t+=d;}}return 0;}
void fP_pawn(int q_sq){int t,pd,dd,cl,cr;if(gS==0){pd=10;dd=20;cl=9;cr=11;t=q_sq+pd;if(gB[t]==_E){if(t>=91&&t<=98){for(int p=0;p<4;p++)fL(M_V(q_sq,t,p,F_R));}else fL(M_V(q_sq,t,0,F_N));if(q_sq>=31&&q_sq<=38){t=q_sq+dd;if(gB[q_sq+pd]==_E&&gB[t]==_E)fL(M_V(q_sq,t,0,F_D));}}t=q_sq+cl;if(gB[t]!=_O&&fP(gB[t])){if(t>=91&&t<=98){for(int p=0;p<4;p++)fL(M_V(q_sq,t,p,F_R|F_C));}else fL(M_V(q_sq,t,0,F_C));}t=q_sq+cr;if(gB[t]!=_O&&fP(gB[t])){if(t>=91&&t<=98){for(int p=0;p<4;p++)fL(M_V(q_sq,t,p,F_R|F_C));}else fL(M_V(q_sq,t,0,F_C));}if(gE!=0){if(q_sq+cl==gE)fL(M_V(q_sq,gE,0,F_X));if(q_sq+cr==gE)fL(M_V(q_sq,gE,0,F_X));}}else{pd=-10;dd=-20;cl=-9;cr=-11;t=q_sq+pd;if(gB[t]==_E){if(t>=21&&t<=28){for(int p=0;p<4;p++)fL(M_V(q_sq,t,p,F_R));}else fL(M_V(q_sq,t,0,F_N));if(q_sq>=81&&q_sq<=88){t=q_sq+dd;if(gB[q_sq+pd]==_E&&gB[t]==_E)fL(M_V(q_sq,t,0,F_D));}}t=q_sq+cl;if(gB[t]!=_O&&fP(gB[t])){if(t>=21&&t<=28){for(int p=0;p<4;p++)fL(M_V(q_sq,t,p,F_R|F_C));}else fL(M_V(q_sq,t,0,F_C));}t=q_sq+cr;if(gB[t]!=_O&&fP(gB[t])){if(t>=21&&t<=28){for(int p=0;p<4;p++)fL(M_V(q_sq,t,p,F_R|F_C));}else fL(M_V(q_sq,t,0,F_C));}if(gE!=0){if(q_sq+cl==gE)fL(M_V(q_sq,gE,0,F_X));if(q_sq+cr==gE)fL(M_V(q_sq,gE,0,F_X));}}}
